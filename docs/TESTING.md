...
### Для NewtonInterpolator

1. Добавьте новую тестовую функцию в `tests/test_newton_interpolator.cpp`:

```cpp
void test_new_feature() {
    test_group("Новая функциональность");
...
2. Вызовите функцию в `run_newton_interpolator_tests()`.

### Для GnuplotVisualizer

Аналогично, добавьте тест в `tests/test_gnuplot_visualizer.cpp` и вызовите в `run_gnuplot_visualizer_tests()`.
...
1. Тесты для `GnuplotVisualizer` требуют наличия gnuplot в системе для полной проверки функциональности. Если gnuplot не установлен, тесты проверяют только отсутствие критических ошибок.

2. Тест `test_interpolate_sin_function()` использует допуск 1e-3, так как полиномиальная интерполяция не-полиномиальных функций имеет ограниченную точность.

## Отладка тестов

Если тест не проходит:

1. Проверьте сообщение об ошибке - оно содержит ожидаемое и полученное значения
2. Убедитесь, что тестовые данные корректны
3. Проверьте допуск (epsilon) для сравнений с плавающей точкой
4. Убедитесь, что тестируемый код компилируется без ошибок

## Контакты и поддержка

При возникновении проблем с тестами проверьте:
- Корректность компиляции (`make test`)
- Наличие всех необходимых файлов
- Соответствие версии компилятора требованиям
